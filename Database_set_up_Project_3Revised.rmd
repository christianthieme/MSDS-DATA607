...
Job data was consumed stored in a staging table and then several tables were normalized.
A denormalized table for the skills was created for the initial ggplot view of the top 10 skills.
I found a great new library to use for this: splitstackshape

Obstacles overall:
•	My local db wouldn’t allow for an insert – had to turn on the option on MySQL to SET GLOBAL local_infile = true;
•	R gave an error for the id column when inserting data.  The workaround was to leave out the auto increment id column and alter the table after the upload.
**I attempted to use the google cloud and I had connectivity issues - this is a goal for a future project.
...
DBI::dbGetQuery(con, "DROP TABLE IF exists DistinctSkills;")
query3 <- "select distinct Skill from skillsforggplot"
results <- DBI::dbSendQuery(con, query3)
db_results <- DBI::dbFetch(results)
DBI::dbWriteTable(con, "DistinctSkills", db_results, overwrite=TRUE)
